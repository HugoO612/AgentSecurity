...
async function planRebuild(
  ...
  const execution = await executeActionPlan('rebuild_environment', operationId, config, [
    ...
    { command: 'verify_checksum', stage: 'verifying_bootstrap' },
    { command: 'install_agent', stage: 'installing_openclaw' },
...
async function executeActionPlan(
  ...
  for (const item of plan) {
    const result = await runTemplateCommand({
      ...
      bundledRootfsChecksum: config.bundledRootfsChecksum,
      bundledRootfsPath: config.bundledRootfsPath,
      bundledAgentArtifactPath: config.bundledAgentArtifactPath,
      bundledBootstrapPath: config.bundledBootstrapPath,
      bundledBootstrapChecksum: config.bundledBootstrapChecksum,
      bundledNodeTarballPath: config.bundledNodeTarballPath,
      bundledNodeTarballChecksum: config.bundledNodeTarballChecksum,
      bundledOpenClawTarballPath: config.bundledOpenClawTarballPath,
      bundledOpenClawTarballChecksum: config.bundledOpenClawTarballChecksum,
      ubuntuVersion: config.ubuntuVersion,
      nodeVersion: config.nodeVersion,
      openClawPackageName: config.openClawPackageName,
  ...
  return finalizeSnapshot(
    {
      ...
      runtime: {
        ...
        processState: 'running',
        lastStartedAt: now,
        installationLocationSummary: `OpenClaw 已安装到专用 Ubuntu ${config.ubuntuVersion} WSL2 环境 ${config.targetDistro}。`,
