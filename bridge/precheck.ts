...
export async function buildPrecheck(config: BridgeConfig) {
  ...
  const checks: PrecheckItem[] = [
    ...
    },
    {
      ...
      message:
        wsl.reason === 'permission_denied'
          ...
          : '后续某些步骤可能请求系统权限。',
      rawDetail: wsl.detail,
      resolutionKind: wsl.reason === 'permission_denied' ? 'user_confirm' : 'user_confirm',
    ...
    },
    {
      ...
      resolutionKind: distro.exists ? 'auto' : 'auto',
      userAction: 'none',
      updatedAt: now,
    },
    {
      code: 'agent_installed',
      status: distro.exists ? 'warning' : 'warning',
...
async function checkDedicatedDistro(targetDistro: string): Promise<DistroStatus> {
  try {
