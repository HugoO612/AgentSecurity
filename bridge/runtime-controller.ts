  ...
  return {
    ...
    args: [
      ...
      [
        ...
        `$runtimeValue=@("TARGET_DISTRO=${escapePowershellString(context.targetDistro)}","AGENT_NAME=OpenClaw","UBUNTU_VERSION=${escapePowershellString(context.ubuntuVersion)}","NODE_VERSION=${escapePowershellString(context.nodeVersion)}","OPENCLAW_INSTALL_SOURCE=npm","OPENCLAW_VERSION_POLICY=latest","ONBOARDING_URL=http://127.0.0.1:18789/") -join "\`n"`,
  ...
  return {
    ...
    args: [
      ...
      [
        ...
        `& wsl.exe -d '${escapePowershellString(context.targetDistro)}' -- /bin/sh -c 'mkdir -p /var/lib/agent-security; echo running >/var/lib/agent-security/state; echo http://127.0.0.1:18789/ >/var/lib/agent-security/onboarding-url'`,
