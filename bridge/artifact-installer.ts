  ...
  return {
    ...
    args: [
      ...
      [
        ...
        `  & wsl.exe -d '${escapePowershellString(context.targetDistro)}' -- sh -lc "mkdir -p /opt/agent-security/current && tar -xf /opt/agent-security/inbox/$name -C /opt/agent-security/current"`,
        '  if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }',
        '} elseif ($lower.EndsWith(".sh")) {',
